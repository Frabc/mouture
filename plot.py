...
def process(csv,threshold,sep=";"):
    # Inputs a cvs and creates a number of graphs in results

    df = pd.read_csv(csv,sep=sep) # data frame (collection par colonne) to read csv with panda and  ; separator
    ...
    for index,row in df.iterrows():   # iterate rows egrener ligne une par une
        # Inputs
        c1  = Vec(row["corner1_x"],row["corner1_y"],row["corner1_z"])
        c2  = Vec(row["corner2_x"],row["corner2_y"],row["corner2_z"])
        c3  = Vec(row["corner3_x"],row["corner3_y"],row["corner3_z"])
        tip = Vec(row["pen_tip_x"],row["pen_tip_y"],row["pen_tip_z"])
        if index==0:                  # pour la ligne 0,  calcul i j et k avec les C1 c2 C3 calcul une seule fois
            ...
            T = Mtx(i,j,k).inv()    # Rebase transformation matrix
...
def main():
    csv = glob.glob("dataset/vid7*.csv")[0]    # define csv as the first doc in data set which name start with videoXX (to be change each time)
    process(csv,1.83,sep=";")
    csv = glob.glob("dataset/vid13*.csv")[0]    # define csv as the first doc in data set which name start with videoXX (to be change each time)
    process(csv,1.84,sep=";")
main()
